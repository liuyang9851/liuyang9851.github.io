---
title: 级数
description: 数项级数的性质与审敛法、正项级数、交错级数与莱布尼茨定理、绝对收敛与条件收敛、幂级数与收敛半径、函数展开成幂级数、傅里叶级数、二元函数可微的充分条件。
---
...
证明：设级数$\sum_{n = 1}^{\infty}u_{n}$的部分和为sn，且sn → s(n → ∞)，则

limn → ∞un = limn → ∞(sn − sn − 1) = limn → ∞sn − limn → ∞sn − 1 = s − s = 0
...
eg.若f(x)的周期为2π，在[−π，π)上的表达式为f(x) = |x|，则其傅里叶级数为
...
其中f(x0 + Δx，y0 + Δy) − f(x0，y0)是全增量，fx′(x0，y0)Δx + fy′(x0，y0)Δy是线性主部，故

[f(x0 + Δx，y0 + Δy) − f(x0，y0)] − [fx′(x0，y0)Δx + fy′(x0，y0)Δy]应为$\sqrt{(\Delta x)^{2} + (\Delta y)^{2}}$的高阶无穷小
